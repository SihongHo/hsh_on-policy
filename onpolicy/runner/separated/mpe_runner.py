...
class MPERunner(Runner):
    ...
    def robust_run(self):
        ...
        for episode in range(episodes): # 3/4 training
            if self.use_linear_lr_decay:
                for agent_id in range(self.num_agents):
                    self.trainer[agent_id].policy.lr_decay(episode, episodes)
        ...
        for episode in range(episodes, int(episodes * 1.25), ): # 1/4 testing
            if self.use_linear_lr_decay:
                for agent_id in range(self.num_agents):
                    self.trainer[agent_id].policy.lr_decay(episode, episodes)
    ...
    def collect_noise(self, step):
        ...
        for agent_id in range(self.num_agents):
            self.trainer[agent_id].prep_rollout()
            share_obs_noise = self.addNoise(self.buffer[agent_id].share_obs[step])
            obs_noise = self.addNoise(self.buffer[agent_id].obs[step])
            value, action, action_log_prob, rnn_state, rnn_state_critic \
                = self.trainer[agent_id].policy.get_actions(share_obs_noise,
                                                            obs_noise,
                                                            self.buffer[agent_id].rnn_states[step],
                                                            self.buffer[agent_id].rnn_states_critic[step],
                                                            self.buffer[agent_id].masks[step])
    ...
    def eval_noise(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            for agent_id in range(self.num_agents):
                self.trainer[agent_id].prep_rollout()
                noise_obs = self.addNosie(np.array(list(eval_obs[:, agent_id])))
                eval_action, eval_rnn_state = self.trainer[agent_id].policy.act(noise_obs,
                                                                                eval_rnn_states[:, agent_id],
                                                                                eval_masks[:, agent_id],
                                                                                deterministic=True)
