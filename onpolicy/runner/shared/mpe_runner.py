...
class MPERunner(Runner):
    ...
    def robust_run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                print("robust average episode rewards is {}".format(train_infos["average_episode_rewards"]))
        ...
        for episode in range(episodes,int(episodes*1.25)):
            # print(episode)
            if self.use_linear_lr_decay:
                self.trainer.policy.lr_decay(episode, episodes)
    ...
    def collect_noise(self, step):
        ...
        value, action, action_log_prob, rnn_states, rnn_states_critic \
            = self.trainer.policy.get_actions(share_obs_noise,
                            obs_noise,
    ...
    def eval_noise(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            self.trainer.prep_rollout()
            eval_obs_noise = self.addNoise(np.concatenate(eval_obs))
            eval_action, eval_rnn_states = self.trainer.policy.act(eval_obs_noise,
                                                np.concatenate(eval_rnn_states),
                                                np.concatenate(eval_masks),
                                                deterministic=True)
            eval_actions = np.array(np.split(_t2n(eval_action), self.n_eval_rollout_threads))
